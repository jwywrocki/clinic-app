                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div className="hidden md:block">
                                <div className="text-xs lg:text-sm font-medium text-gray-600">
                                    <span className="hidden lg:inline">Samodzielny Publiczny Zakład Opieki Zdrowotnej</span>
    ...
    return (
        <header className="bg-white shadow-lg fixed top-0 left-0 right-0 z-50" role="banner">
            <div className="container mx-auto px-4">
                <div className="flex items-center justify-between h-20">
                    {/* Logo */}
                    <Link href="/" className="flex items-center gap-3 text-xl font-bold text-gray-900 focus:outline-none focus:ring-4 focus:ring-blue-600 rounded-lg p-2 flex-shrink-0">
                        <Image src="/images/logo.png" alt="Logo" width={58} height={74} className="w-auto h-auto max-w-[58px] max-h-[58px]" />
                        <div className="hidden md:block">
                            <div className="text-xs lg:text-sm text-gray-600">
                                <span className="hidden xl:inline">Samodzielny Publiczny Zakład Opieki Zdrowotnej</span>
                    ...
                    {/* Desktop Navigation */}
                    <nav ref={menuRef} className="hidden lg:block flex-shrink-0" role="navigation" aria-label="Nawigacja główna">
                                    ...
                                    <li key={item.id} className="relative menu-item" onMouseEnter={() => hasChildren && handleMouseEnter(item.id)} onMouseLeave={handleMouseLeave}>
                                        {item.url ? (
                                            ...
                                            <button
                                                ...
                                                `}
                                                onKeyDown={(e) => handleKeyDown(e, item.id, Boolean(hasChildren))}
                                                onClick={() => hasChildren && setActiveDropdown(activeDropdown === item.id ? null : item.id)}
                                                aria-expanded={hasChildren ? isDropdownOpen : undefined}
                                                aria-haspopup={hasChildren ? 'menu' : undefined}
                                            >
                                                <span>{item.title}</span>
                                                ...
                                                >
                                                    ...
                                                    {item.children?.map((child) => {
                                                        ...
                                                        return child.url ? (
                                                            ...
                                                            <span key={child.id} className="block px-4 py-3 text-sm font-medium text-gray-500" role="menuitem">
                                                                {child.title}
                                                            </span>
                                                        );
                                                    })}
                                                ...
                                                {item.children.map((child) => (
                                                    <li key={child.id}>
                                                        {child.url ? (
                                                            ...
                                                            <span className="block py-2 px-4 text-sm text-gray-500 rounded-lg">{child.title}</span>
                                                        )}
                                                    </li>
                                                ))}
                                            </ul>
