                        ...
                        <div className="space-y-2">
                            <div className="flex items-center justify-between">
                                ...
                                {settings.email_smtp_password && (
                                    <Badge variant={isEncrypted(settings.email_smtp_password) ? 'default' : 'secondary'} className="text-xs">
                                        {isEncrypted(settings.email_smtp_password) ? '🔒 Zaszyfrowane' : '📝 Nowe hasło'}
                                    </Badge>
                                )}
                            </div>
                            <div className="relative">
                                <Input
                                    ...
                                    value={isEncrypted(settings.email_smtp_password) ? '••••••••••••••••' : settings.email_smtp_password}
                                    onChange={(e) => updateSetting('email_smtp_password', e.target.value)}
                                    placeholder={isEncrypted(settings.email_smtp_password) ? 'Wprowadź nowe hasło aby zmienić' : 'Wprowadź hasło SMTP'}
                                    className="pr-10"
                                />
                                <button
                                    type="button"
                            ...
                            {settings.email_smtp_password && !isEncrypted(settings.email_smtp_password) && (
                                <p className="text-xs text-blue-600">💡 Hasło zostanie automatycznie zaszyfrowane przy zapisie</p>
                            )}
                            {isEncrypted(settings.email_smtp_password) && <p className="text-xs text-green-600">✅ Hasło jest bezpiecznie przechowywane w zaszyfrowanej formie</p>}
                        </div>
                        <div className="space-y-2">
                ...
                <CardContent className="space-y-4">
                    <div className="space-y-2">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                            Aby uzyskać link do osadzenia, przejdź do Google Maps, znajdź lokalizację, kliknij "Udostępnij" → "Osadź mapę" i skopiuj URL z iframe src
                        </p>
                    </div>
                    {settings.google_maps_embed_url && (
                        <div className="p-3 bg-green-50 rounded-lg">
                            <p className="text-sm text-green-800">✅ Mapa zostanie wyświetlona na dole strony kontaktu</p>
                        </div>
                    )}
                </CardContent>
                <div className="border-t bg-gray-50 px-6 py-4 rounded-b-lg">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div className="p-3 bg-blue-50 rounded-lg">
                                <div className="flex items-center justify-between">
                                    ...
                                    <Badge variant={schedulerStatus.backup_enabled ? 'default' : 'secondary'}>{schedulerStatus.backup_enabled ? 'Włączone' : 'Wyłączone'}</Badge>
                                </div>
                                <p className="text-xs text-blue-600 mt-1">Częstotliwość: {schedulerStatus.backup_frequency}</p>
                            </div>
