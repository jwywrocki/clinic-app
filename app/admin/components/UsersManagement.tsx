...
interface UsersManagementProps {
  ...
  onSave: (user: Partial<User>) => Promise<{ success: boolean; error?: string } | void>;
    ...
    if (editingUser) {
      ...
      try {
        ...
        if (result && !result.success) {
          toast({
            title: 'Błąd',
            description: result.error || 'Nie udało się zapisać użytkownika',
            variant: 'destructive',
          });
          return;
        }
        toast({
          title: 'Sukces',
          description: editingUser.id ? 'Użytkownik zaktualizowany' : 'Użytkownik dodany',
          variant: 'success',
        });
        setEditingUser(null);
        router.refresh();
        ...
        toast({
          title: 'Błąd',
          description: error.message || 'Nie udało się zapisać użytkownika',
          variant: 'destructive',
        });
      }
    }
  ...
  return (
    <AnimatedSection animation="fadeInUp">
                    ...
                    <tr key={user.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        ...
                        <Button
                              ...
                              onConfirm: async () => {
                                try {
                                  ...
                                  toast({
                                    title: 'Błąd',
                                    description:
                                      error.message || 'Nie udało się usunąć użytkownika',
                                    variant: 'destructive',
                                  });
                                }
                              },
                          ...
                          disabled={
                            user.username === currentUser?.username || user.username === 'jaqb'
                          }
      ...
      <ConfirmDialog
        ...
        title="Usuń użytkownika"
        description="Czy na pewno chcesz usunąć tego użytkownika? Tej operacji nie można cofnąć."
