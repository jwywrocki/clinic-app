...
export function SurveysManagement({
  ...
  const handleAddQuestion = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (
          (newQuestion.type === 'single' || newQuestion.type === 'multi') &&
          ...
          for (let i = 0; i < validOptions.length; i++) {
            ...
            await fetch(
              `/api/surveys/${selectedSurvey.id}/questions/${newQuestionResult.id}/options`,
              {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
      ...
      onConfirm: async () => {
        try {
          const response = await fetch(
            `/api/surveys/${selectedSurvey.id}/questions/${questionId}`,
            {
              method: 'DELETE',
            }
          );
  ...
  const handleAddOptionToExistingQuestion = async (questionId: string) => {
    ...
    try {
      const response = await fetch(
        `/api/surveys/${selectedSurvey.id}/questions/${questionId}/options`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify(optionData),
  ...
  const handleDeleteQuestionOption = (optionId: string) => {
    ...
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          const response = await fetch(
            `/api/surveys/${selectedSurvey.id}/questions/dummy/options/${optionId}`,
            {
              method: 'DELETE',
            }
          );
            ...
            {isNew ? (
              ...
              <div className="space-y-2">
                ...
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  onClick={() => handleAddOptionToExistingQuestion(question!.id)}
                >
                  <Plus className="h-4 w-4 mr-2" />
                  Dodaj opcję
                </Button>
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {surveys.map(survey => (
                  <tr key={survey.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      {survey.updated_at
                        ? new Date(survey.updated_at).toLocaleDateString('pl-PL')
                        : '-'}
                    </td>
              ...
              {selectedSurvey && (
                <div className="space-y-6">
                  ...
                  <div className="flex justify-end gap-2 pb-4 border-b">
                    ...
                    <Button
                      onClick={() => handleSaveFullSurvey(selectedSurvey)}
                                    ...
                                    {question.options.map((option, optIndex) => (
                                      ...
                                      >
                                        ...
                                        <Input
                                          value={option.text}
                                          onChange={e =>
                                            handleUpdateQuestionOption(option.id, e.target.value)
                                          }
                                          className="text-sm"
                                        />
                                        <Button
