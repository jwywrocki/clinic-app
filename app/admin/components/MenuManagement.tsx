...
interface MenuManagementProps {
  menuItems: MenuItem[];
  onSave: (
    menuItem: LoosePartial<MenuItem>
...
export function MenuManagement({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingItem && formData.order_position !== editingItem.order_position) {
        ...
        await onUpdateOrder?.(updatedItems);
      ...
      toast({
        title: 'Sukces',
        description: editingItem ? 'Pozycja menu zaktualizowana' : 'Pozycja menu dodana',
        variant: 'success',
      });
      router.refresh();
    } catch (error: any) {
      toast({
        title: 'Błąd',
        description: error.message || 'Nie udało się zapisać pozycji menu',
        variant: 'destructive',
      });
    }
  ...
  const reorderMenuItems = (updatedItem: Partial<MenuItem>): MenuItem[] => {
    ...
    if (oldParentId !== parentId) {
      const oldGroupItems = menuItems.filter(
        item => (item.parent_id || null) === oldParentId && item.id !== editingItem.id
      );
    ...
    const currentGroupItems = menuItems.filter(
      item => (item.parent_id || null) === parentId && item.id !== editingItem.id
    );
  ...
  const handleDelete = (id: string) => {
    setConfirmDialog({
      ...
      onConfirm: async () => {
        try {
          await onDelete(id);
          toast({ title: 'Sukces', description: 'Pozycja menu usunięta', variant: 'success' });
          router.refresh();
        } catch (error: any) {
          toast({
            title: 'Błąd',
            description: error.message || 'Nie udało się usunąć pozycji menu',
            variant: 'destructive',
          });
        }
      },
  ...
  return (
    <AnimatedSection>
        ...
        <CardContent>
          ...
          {isFormVisible && (
            <Card className="mb-6">
              <CardHeader>
                <CardTitle>
                  {editingItem ? 'Edytuj element menu' : 'Dodaj nowy element menu'}
                </CardTitle>
              </CardHeader>
              <CardContent>
                      ...
                      <div className="flex items-center space-x-2 mb-2">
                        <input
                          ...
                          onChange={e =>
                            setFormData({
                              ...formData,
                              no_link: e.target.checked,
                              url: e.target.checked ? '' : formData.url,
                            })
                          }
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      ...
                      <select
                        ...
                        onChange={e =>
                          setFormData({ ...formData, parent_id: e.target.value, order_position: 1 })
                        }
                      >
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {organizedMenuItems.map(item => {
                  ...
                  return (
                    ...
                    >
                      <td
                        className={`px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 ${isChild ? 'pl-16' : ''}`}
                      >
                        <div className="flex items-center">
                          {isChild && <span className="text-gray-400 mr-2">↳</span>}
                          <span className={isChild ? 'font-normal' : 'font-semibold'}>
                            {item.title}
                          </span>
                        </div>
                      </td>
      ...
      <ConfirmDialog
        ...
        title="Usuń element menu"
        description="Czy na pewno chcesz usunąć ten element menu? Tej operacji nie można cofnąć."
